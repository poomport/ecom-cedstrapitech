...
const Cart = () => {
  ...
  const handlePayment = async () => {
    try {
      ...
      const sessionId = res.data?.strapiSession?.id;
      ...
      if (sessionId) {
        ...
        console.error('Missing or invalid Stripe session ID in the response:', res);
      }
      ...
      <div className='px-6'>
        {cart.length >= 1 ? (
          <div className='flex justify-between gap-x-4'>
            ...
            <button onClick={handlePayment} className='btn btn-accent hover:bg-accent-hover text-primary flex-1 px-2 gap-x-2'>Checkout <IoArrowForward className='text-lg' /></button>
          </div>) : (
